...
import { getMarketData, getBrief } from '../../services/marketService';
...
export default function Dashboard() {
  ...
  const {
    ...
    queryFn: () => unwrapService(getBrief()),
  });
  ...
  const { assets, market, keyEvent } = marketData;
  ...
  const relevantBrief = briefPoints.filter((point) => {
  ...
  return (
    <div className={styles.dashboard}>
        ...
        <div className={styles.assetCards}>
          {filteredAssets.map((asset) => (
            <div key={asset.id} className={styles.assetCard}>
              ...
              <Sparkline history={asset.history} positive={asset.change >= 0} />
      ...
      <div className={styles.dailyBrief}>
        <h3>{t.dailyBrief}</h3>
        {briefLoading && <LoadingSkeleton variant="list" />}
        {briefError && <ErrorMessage message={briefError.message} onRetry={refetchBrief} />}
